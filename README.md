# AntiAfkKick（繁中移植版 · TC13） / Traditional-Chinese Port

> 防止因閒置被遊戲自動踢出。裝了就忘，免設定。<br>
> Prevents being auto-kicked from FFXIV for inactivity. Install and forget — no config.

**繁體中文**：這是 **[AntiAfkKick](https://github.com/NightmareXIV/AntiAfkKick)** 的繁體中文客戶端移植版，對應 **FFXIV 7.20 / yanmucorp Dalamud API13（.NET 9）**。本專案僅做相容性移植，**非官方、非原作維護**；所有原始功能與設計著作權歸原作者 **NightmareXIV**。

**English**: A Traditional-Chinese-client port of **[AntiAfkKick](https://github.com/NightmareXIV/AntiAfkKick)** targeting **FFXIV 7.20 / yanmucorp Dalamud API13 (.NET 9)**. Compatibility port only — **unofficial and not maintained by the original author**. All original work © **NightmareXIV**.
...
3. 貼上下列網址並按 **+** 儲存：
   ...
   ```
4. 輸入 `/xlplugins`，搜尋 **AntiAfkKick (TC13)** → 安裝 → 啟用。
...
3. Add this URL and save with **+**:
   ...
   https://raw.githubusercontent.com/lilasrepo/DalamudPlugins/main/pluginmaster.json
   ```
4. Type `/xlplugins`, search **AntiAfkKick (TC13)** → Install → Enable.
